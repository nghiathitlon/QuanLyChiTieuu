...
.category-card {
    ...
    display: flex;                /* chữ + actions nằm ngang */
    flex-direction: row;
    justify-content: space-between; /* đẩy actions sang phải */
    align-items: flex-start;        /* canh chữ và nút */
    word-break: break-word;         /* chữ dài xuống dòng */
    min-height: 50px;
}
...
.category-card h4 {
    ...
    min-width: 0;                  /* cho word-break hoạt động trong flex */
}
...
.category-card .actions a {
    ...
    text-decoration: none;
    color: #007bff;
    white-space: nowrap;            /* tránh xuống dòng trong nút */
}
...
document.getElementById('editCategoryType').value = card.classList.contains('expense')?'expense':'income';
...
document.getElementById('message').innerHTML = '<span style="color:red">'+res.msg+'</span>';
}
})
});
